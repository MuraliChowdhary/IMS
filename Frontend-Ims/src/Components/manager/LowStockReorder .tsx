    ...
    const fetchLowStock = async () => {
        try {
        const res = await axios.get("http://localhost:5000/api/manager/lowstock", {
            headers: {
    ...
    const handleReorder = async () => {
        setReordering(true);
        try {
        const res = await axios.post("http://localhost:5000/api/manager/reorder", {}, {
            headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
        });
        ...
        {response && (
            ...
            {response.details.map((detail, index) => (
                ...
                <CardContent className="text-sm space-y-1">
                    ...
                    <Badge variant="secondary">{detail.status.priorityStatus}</Badge>
                </CardContent>
                </Card>
            ))}
            </div>
